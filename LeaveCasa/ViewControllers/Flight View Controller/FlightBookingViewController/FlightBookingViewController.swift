...
class FlightBookingViewController: UIViewController {
    ...
    func openDateCalendar(_ textField: UITextField) {
        if let calendar = ViewControllerHelper.getViewController(ofType: .WWCalendarTimeSelector) as? WWCalendarTimeSelector {
            ...
            calendar.optionCurrentDate = isDoB ? Date.date(year: Date().year - 12, month: Date().month, day: Date().day) : Date()
            ...
            DispatchQueue.main.async {
                ...
                WSManager.wsCallFetchFlightFareDetails(params, success: { (resultFareRules, resultFareQuotes) in
                    ...
                    if let results = resultFareQuotes[WSResponseParams.WS_RESP_PARAM_RESULTS_CAP] as? [String:AnyObject] {
                        ...
                        if let flight = Mapper<Flight>().map(JSON: results) as Flight? {
                            if isReturning {
                                self.returningFlights = flight
                            } else {
                                self.flights = flight
                            }
                            self.setupData()
